...
const toLastNames = (people) => {
  return people.map((person) => `${person.firstName} ${person.lastName}`); // Solution code here...
};
...
const addPurchases = (arr) => {
  return arr.reduce((previous, current) => previous + current.purchasePrice, 0); // Solution code here...
};
